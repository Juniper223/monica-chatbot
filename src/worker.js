...
function renderClinics(list) {
  ...
  if (!list.length) { tbody.innerHTML = '<tr><td colspan="5" class="empty">No clinics found</td></tr>'; return; }
...
const LIST_FIELDS = new Set(['locations','cost','payment','insurance_networks',
  'named_modalities','addictions_treated','rehab_type','accessibility']);

function parseList(val) {
  if (!val) return '';
  const s = String(val).trim();
  if (!s.startsWith('[')) return s;
  // Strip outer brackets, split on commas, remove surrounding quotes from each item
...
async function saveClinic(e) {
  e.preventDefault();
  const clinic = {};
  FIELDS.forEach(f => { const el = document.getElementById('f-'+f); if (el) clinic[f] = el.value.trim(); });
  if (clinic.id) clinic.id = parseInt(clinic.id); else delete clinic.id;
  const btn = document.getElementById('save-btn');
  btn.textContent = 'Saving...'; btn.disabled = true;
  try {
    const r = await fetch('/admin/api/clinics?pw=' + encodeURIComponent(PW), {
      method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(clinic)
    });
    const d = await r.json();
    if (d.ok) {
      if (clinic.id) { const idx = allClinics.findIndex(c => c.id === clinic.id); if (idx >= 0) allClinics[idx] = {...allClinics[idx],...clinic}; }
...
async function del(id) {
  ...
  try {
    await fetch('/admin/api/clinics?pw=' + encodeURIComponent(PW) + '&id=' + id, { method: 'DELETE' });
    allClinics = allClinics.filter(x => x.id !== id);
